...
const Register = () => {
  ...
  }, [user, cart, navigate, isCheckoutRedirect, dispatch]);
    ...
    <div className='flex'>
      <div className='w-full md:w-1/2 flex flex-col justify-center items-center p-8 md:p-12'>
        <form onSubmit={handleSubmit} className='w-full max-w-md bg-white p-8 rounded-lg border shadow-sm'>
          ...
          <button type="submit" className='w-full text-white bg-black p-2 rounded-lg font-semibold hover:bg-gray-800 transition'>
            {loading ? "loading" : "Sign Up"}
          </button>
          <p className='mt-6 text-center text-sm'>
            Don't have an account?{" "}
            <Link to={`/login?redirect=${encodeURIComponent(redirect)}`} className="text-blue-500">Login In</Link>
          </p>
        </form>
      </div>
